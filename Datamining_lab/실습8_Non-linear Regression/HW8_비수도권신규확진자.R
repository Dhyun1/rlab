...
for (j in 1:length(deg)){  
  for (i in seq_len(k)){
    fit<- loess(비수도권.신규확진자~Days_after_First_Cases,data=train[unlist(folds[-i]),],span=deg[j],
...
loess_fit=loess(비수도권.신규확진자~Days_after_First_Cases,data=train,span=0.1,
                   control=loess.control(surface="direct")) 
...
ggsave(file="비수도권(1120)_loess.jpg",width=30,height=15)
dev.off()
...
for (j in df){  
  for (i in seq_len(k)){ #i=1,2,3, .. ,9,10
    fit<- gam(비수도권.신규확진자~ns(Days_after_First_Cases,j),data=train[unlist(folds[-i]),]) 
    val_i<-train[unlist(folds[i]),]
...
gam1_fit<-glm(비수도권.신규확진자~ns(Days_after_First_Cases,4),data=train)
gam1_train_fitted<-gam1_fit$fitted.values
(training_MSE=mean((train$비수도권.신규확진자-gam1_train_fitted)^2))
gam1_test_fitted<-predict(gam1_fit,newdata=list("Days_after_First_Cases"=test$Days_after_First_Cases))
(test_MSE=mean((test$비수도권.신규확진자-gam1_test_fitted)^2))
...
ggsave(file="비수도권(1120)_gam1.jpg",width=30,height=15)
dev.off()
...
for (i in cut){
  ...
  step_fit = glm(비수도권.신규확진자~tmp, data = train)
...
ggsave(file="비수도권(1120)_step.jpg",width=30,height=15)
dev.off()

#poisson regression (note 비수도권신규확진자 is non-negative integer)

poisson_fit = glm(비수도권.신규확진자~Days_after_First_Cases, family = 'poisson', data = train, maxit = 100)
poisson_train_fitted<-poisson_fit$fitted.values
(training_MSE<-mean((train$비수도권.신규확진자-poisson_train_fitted)^2))
poisson_test_fitted<-predict(poisson_fit,data.frame("Days_after_First_Cases"=test$Days_after_First_Cases),
                             type = "r")
(test_MSE<-mean((test$비수도권.신규확진자-poisson_test_fitted)^2))
...
ggsave(file="비수도권(1120)_poisson.jpg",width=30,height=15)
dev.off()
...
ggsave(file="비수도권(1120).jpg",width=30,height=15)
dev.off()

#Further analysis : change date, construct new set of train, test data
#9:1 split, do same analysis above
train1<-fulldata[1:round(nrow(fulldata)*0.9),]
test1<-fulldata[-(1:round(nrow(fulldata)*0.9)),]
nrow(train1) #1~282, 283~313 split
...
for (j in deg){  
  for (i in seq_len(k)){ #i=1,2,3, .. ,9,10
    fit<- lm(비수도권.신규확진자~poly(Days_after_First_Cases,j),data=train1[unlist(folds[-i]),])
...
poly_fit=lm(비수도권.신규확진자~poly(Days_after_First_Cases,5),data=train1) 
(training_MSE<-mean((train1$Days_after_First_Cases-poly_fit$fit)^2))
(poly_train1_fitted<-poly_fit$fit)
(poly_test1_fitted<-predict(poly_fit,newdata=list("Days_after_First_Cases"=test1$Days_after_First_Cases),se=TRUE)$fit)
...
fulldata%>%
  ...
  scale_color_discrete(name = "Model", labels = c("poly"))+
  labs(x="Days after first cases",y="비수도권신규확진자",title="비수도권신규확진자(polynomial)")
ggsave(file="비수도권(1027)_polynomial.jpg",width=20,height=10)
dev.off()

#10-fold cv to determine spar for smooth splines
set.seed(1)
smooth.spline(train1$Days_after_First_Cases,train1$비수도권.신규확진자,cv=TRUE) 
smooth_fit<-smooth.spline(train1$Days_after_First_Cases,train1$비수도권.신규확진자,
                          spar = 0.17)
smooth_train1_fitted<-predict(smooth_fit,train1$Days_after_First_Cases)$y
(training_MSE<-mean((train1$비수도권.신규확진자-smooth_train1_fitted)^2))
smooth_test1_fitted<-predict(smooth_fit,test1$Days_after_First_Cases)$y
(test_MSE<-mean((test1$비수도권.신규확진자-smooth_test1_fitted)^2))
...
fulldata%>%
  ...
  scale_color_discrete(name = "Model", labels = c("smooth"))+
  labs(x="Days after first cases",y="비수도권신규확진자",title="비수도권신규확진자(smooth)")
ggsave(file="비수도권(1027)_smooth.jpg",width=20,height=10)
dev.off()
...
for (j in 1:length(deg)){  
  for (i in seq_len(k)){ #i=1,2,3, .. ,9,10
    fit<- loess(비수도권.신규확진자~Days_after_First_Cases,data=train1[unlist(folds[-i]),],span=deg[j],
                   control=loess.control(surface="direct")) 
    val_i<-train1[unlist(folds[i]),]
...
loess_fit=loess(비수도권.신규확진자~Days_after_First_Cases,data=train1,span=0.1,
                   control=loess.control(surface="direct")) 
