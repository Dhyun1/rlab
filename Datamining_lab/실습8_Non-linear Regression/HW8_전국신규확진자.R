...
fulldata%>%
  ...
  geom_line(aes(y=전국.신규확진자))+
  geom_line(aes(y=poly,colour="a"),data=poly_fitted)+
  scale_color_discrete(name = "Model", labels = c("poly"))+
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(polynomial)")
ggsave(file="전국(1120)_polynomial.jpg",width=30,height=15)
dev.off()

#10-fold cv to determine spar for smooth splines
set.seed(1)
smooth.spline(train$Days_after_First_Cases,train$전국.신규확진자,cv=TRUE) #choose spar=0.11
smooth_fit<-smooth.spline(train$Days_after_First_Cases,train$전국.신규확진자, spar = 0.11)
...
fulldata%>%
  ...
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(smooth)")
ggsave(file="전국(1120)_smooth.jpg",width=30,height=15)
dev.off()
...
fulldata%>%
  ...
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(loess)")
ggsave(file="전국(1120)_loess.jpg",width=30,height=15)
dev.off()
...
fulldata%>%
  ...
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(gam1)")
ggsave(file="전국(1120)_gam1.jpg",width=30,height=15)
dev.off()
...
step_fit$xlevels
(step_fit$coefficients[5]+step_fit$coefficients[1])
step_test_fitted<-rep(94,7)
...
fulldata%>%
  ...
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(step)")
ggsave(file="전국(1120)_step.jpg",width=30,height=15)
dev.off()
...
fulldata%>%
  ...
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(poisson)")
ggsave(file="전국(1120)_poisson.jpg",width=30,height=15)
dev.off()
...
fulldata%>%
  ...
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(종합)")
ggsave(file="전국(1120).jpg",width=30,height=15)
dev.off()
...
for (j in deg){  
  for (i in seq_len(k)){ #i=1,2,3, .. ,9,10
    fit<- lm(전국.신규확진자~poly(Days_after_First_Cases,j),data=train1[unlist(folds[-i]),])
...
poly_fit=lm(전국.신규확진자~poly(Days_after_First_Cases,5),data=train1) 
...
fulldata%>%
  ...
  scale_color_discrete(name = "Model", labels = c("poly"))+
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(polynomial)")
ggsave(file="전국(0825)_polynomial.jpg",width=30,height=15)
dev.off()

#10-fold cv to determine spar for smooth splines
set.seed(1)
smooth.spline(train1$Days_after_First_Cases,train1$전국.신규확진자,cv=TRUE) 
smooth_fit<-smooth.spline(train1$Days_after_First_Cases,train1$전국.신규확진자,
                          spar = 0.22)
smooth_train1_fitted<-predict(smooth_fit,train1$Days_after_First_Cases)$y
(training_MSE<-mean((train1$전국.신규확진자-smooth_train1_fitted)^2))
smooth_test1_fitted<-predict(smooth_fit,test1$Days_after_First_Cases)$y
(test_MSE<-mean((test1$전국.신규확진자-smooth_test1_fitted)^2))
...
fulldata%>%
  ...
  scale_color_discrete(name = "Model", labels = c("smooth"))+
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(smooth)")
ggsave(file="전국(0825)_smooth.jpg",width=30,height=15)
dev.off()
...
for (j in 1:length(deg)){  
  for (i in seq_len(k)){ #i=1,2,3, .. ,9,10
    fit<- loess(전국.신규확진자~Days_after_First_Cases,data=train1[unlist(folds[-i]),],span=deg[j],
                    control=loess.control(surface="direct")) 
    val_i<-train1[unlist(folds[i]),]
...
loess_fit=loess(전국.신규확진자~Days_after_First_Cases,data=train1,span=0.1,
                    control=loess.control(surface="direct")) 
...
fulldata%>%
  ...
  scale_color_discrete(name = "Model", labels = c("loess"))+
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(loess)")
ggsave(file="전국(0825)_loess.jpg",width=30,height=15)
dev.off()
...
for (j in df){  
  for (i in seq_len(k)){ #i=1,2,3, .. ,9,10
    fit<- gam(전국.신규확진자~ns(Days_after_First_Cases,j),data=train1[unlist(folds[-i]),]) 
    val_i<-train1[unlist(folds[i]),]
    pred <- predict(fit, newdata=list("Days_after_First_Cases"=val_i$Days_after_First_Cases)) #se=FALSE, no $fit required
    mse[i] <- mean((pred - val_i$전국.신규확진자)^2)
  }
...
gam1_fit<-glm(전국.신규확진자~ns(Days_after_First_Cases,5),data=train1)
gam1_train1_fitted<-gam1_fit$fitted.values
(training_MSE=mean((train1$전국.신규확진자-gam1_train1_fitted)^2))
gam1_test1_fitted<-predict(gam1_fit,newdata=list("Days_after_First_Cases"=test1$Days_after_First_Cases))
(test_MSE=mean((test1$전국.신규확진자-gam1_test1_fitted)^2))
...
fulldata%>%
  ...
  scale_color_discrete(name = "Model", labels = c("gam1"))+
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(gam1)")
ggsave(file="전국(0825)_gam1.jpg",width=30,height=15)
dev.off()
...
for (i in cut){
  ...
  step_fit = glm(전국.신규확진자~tmp, data = train1)
...
fulldata%>%
  ...
  scale_color_discrete(name = "Model", labels = c("step"))+
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(step)")
ggsave(file="전국(0825)_step.jpg",width=30,height=15)
dev.off()

#poisson regression 
poisson_fit = glm(전국.신규확진자~Days_after_First_Cases, family = 'poisson', data = train1, maxit = 100)
(poisson_train1_fitted<-poisson_fit$fitted.values)
(training_MSE<-mean((train1$전국.신규확진자-poisson_train1_fitted)^2))
(poisson_test1_fitted<-exp(predict(poisson_fit,data.frame("Days_after_First_Cases"=test1$Days_after_First_Cases))))

(test_MSE<-mean((test1$전국.신규확진자-poisson_test1_fitted)^2))
...
fulldata%>%
  ...
  scale_color_discrete(name = "Model", labels = c("poisson"))+
  labs(x="Days after first cases",y="전국신규확진자",title="전국신규확진자(poisson)")
ggsave(file="전국(0825)_poisson.jpg",width=30,height=15)
dev.off()
...
ggsave(file="전국(0825).jpg",width=30,height=15)
dev.off()